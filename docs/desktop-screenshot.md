...
`installScreenshotShortcut` 在主窗口初始化时注册设备级快捷键：macOS `Command+Shift+A`，Windows `Control+Shift+A`。优先使用当前聚焦且受信任的会话窗口，否则使用主窗口作为截图所属窗口；直接由主进程启动截图，不依赖页面事件订阅或对话输入框。账号作用域和主窗口就绪后，录音、设置、未打开对话、最小化和后台运行均可触发，不主动激活主窗口。
...
### 按触发入口区分完成行为（2026-09-22）

- 输入框截图按钮：沿用 `capture` IPC，完成后将 PNG 返回发起的输入框，作为待发送附件，不自动发送，也不覆盖剪贴板。
- 系统快捷键：调用主进程的 `captureClipboard`，完成后写入系统剪贴板，不向输入框发送触发事件或图片结果；即使当前正在私聊，也不会生成附件。
- 目的地在创建截图会话时固定，不能由编辑窗口通过 IPC 更改。快捷键重复触发不打断已有截图；取消、来源失效和账号切换不改剪贴板；写入失败保留编辑器供重试。完成时同步校验 owner 权限，避免等待定时清理期间接收失效结果。
- 截图 UI/保存功能沿用已有编辑器。原生能力和账号作用域就绪前不发起截图；退出应用注销快捷键。

根因与排查路径：从 `globalShortcut` 注册追踪到窗口 `shortcut-trigger` 广播，再追踪到 `ArkmeComposerScreenshotButton` 的订阅和 `active/disabled/isCurrent` 检查。原实现以对话输入框为快捷键执行者，并复用其 `onFile` 附件回调，导致非对话板块失效、私聊快捷键误生成附件。现将系统快捷键入口移入主进程。

验证：客户端截图/快捷键/窗口几何等 55 项测试、插件截图相关 69 项测试、两仓 TypeScript 检查和构建通过。先观察旧实现回归失败，再实现并复验。用 Electron 模拟接口验证了剪贴板目标像素、取消/失效不写入、失败重试、重复触发和最小化；尚未执行本轮 macOS/Windows 完整客户端实机验收。插件构建使用 package.json 中相同子命令直接执行，避免 pnpm 自动安装改写复用的依赖目录。
